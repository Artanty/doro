...
---
когда событие заканчивается - триггерится его hook on complete.
по умолчанию - это скрипт. хранить в бд следующейй событие - плохо, при смене порядка придется следить за этим.
так что это скрипт, который сам сходит и посмотрит на следующее событие.
---
отдавать в tik@ только тикающие ивенты.
finish:
tik@ решает, когда ивент заканчивается и удаляет его у себя.
при записи в таблицу eventState doro@ обновляет configHash.
pause:
doro@ обновляет состояние события в eventState,
просит tik@ удалить событие
doro сам возвращает paused-state
stopped (initial state):
doro сам возвращает stopped-state

todo: обогащать стейт события не только из tik@, но и из doro@
---
...
получение состояния.
Всегда возвращаются все актуальные ивенты
-
