...
export class EventStateController {
     ...
     */
    static async createOrUpdateEventState(eventId: any, connectionId: any, state: any, userHandler: any) {
        ...
        try {
            ...
            const [result] = await connection.execute(
                `INSERT INTO eventState (eventId, connectionId, state, userHandler) 
                 VALUES (?, ?, ?, ?) 
                 ON DUPLICATE KEY UPDATE state = ?, userHandler = ?, updated_at = CURRENT_TIMESTAMP`,
                [eventId, connectionId, state, userHandler, state, userHandler]
            );
     ...
     * Update event state
     */
    static async updateEventState(eventId: any, connectionId: any, state: any, userHandler: any) {
        const pool = createPool();
        const connection = await pool.getConnection();
        try {
            await connection.beginTransaction();
            ...
            const [result] = await connection.execute(
                `UPDATE eventState 
                 SET state = ?, userHandler = ?, updated_at = CURRENT_TIMESTAMP 
                 WHERE eventId = ? AND connectionId = ?`,
                [state, userHandler, eventId, connectionId]
            );
